...
namespace base
{
	...
	inline void PriorityQueue<NODE>::push(NODE *node)
	{
		...
		percolate_up((uint32_t)_data.size() - 1);
	}
	...
	void PriorityQueue<NODE>::remove(NODE *node)
	{
		...
		uint32_t index = node->_pq_index;
		uint32_t n = (uint32_t)_data.size() - 1;
	...
	template<typename NODE>
	void PriorityQueue<NODE>::adjust_node(uint32_t index)
	{
		assert(index < (uint32_t)_data.size());
	...
	template<typename NODE>
	void PriorityQueue<NODE>::percolate_up(uint32_t index)
	{
	...
	template<typename NODE>
	void PriorityQueue<NODE>::percolate_down(uint32_t index)
	{
		NODE *node = _data[index];
